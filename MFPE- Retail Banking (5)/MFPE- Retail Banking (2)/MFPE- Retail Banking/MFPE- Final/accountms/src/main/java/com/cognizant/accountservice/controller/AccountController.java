...
public class AccountController { 
	...
	@PostMapping("/loan")
	public ResponseEntity<Account> loan(@RequestHeader("Authorization") String token, @RequestBody AccountInput accInput) {
		accountServiceImpl.hasPermission(token);
		System.out.println(accInput.getAmount());
		try {
			// Calling the makeWithdraw API from Transaction microservice 
			transactionFeign.makeLoan(token, accInput);
			System.out.println(accInput.getAmount());
		...
		System.out.println(accInput.getAmount());
		Account newUpdateAccBal = accountServiceImpl.applyLoan(accInput); //Completed
		List<Transaction> list = transactionFeign.getTransactionsByAccId(token, accInput.getAccountId());
		newUpdateAccBal.setTransactions(list);
		LOGGER.info("amount Loan sucessful");
		return new ResponseEntity<>(newUpdateAccBal, HttpStatus.OK);
	}
